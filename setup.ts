...
import { loadDefaultOrganization } from "@utilities/loadDefaultOrg";
...
export async function postgresDB(): Promise<void> {
    ...
    try {
        let url = await checkExistingPostgresDB();
        let isConnected = url !== null;
        ...
        const envFilePath = process.env.NODE_ENV === "test" ? ".env_test" : ".env";
        DB_URL = url;
...
async function main(): Promise<void> {
    ...
    if (isDockerInstallation) {
        const DB_URL = "postgres://postgres:password@localhost:5432/talawa_api";
        ...
        config.API_POSTGRES_URL = DB_URL;
        ...
        process.env.API_POSTGRES_URL = DB_URL;
    ...
    if (!isDockerInstallation) {
        ...
        if (process.env.API_POSTGRES_URL) {
            console.log(
    ...
    if (!isDockerInstallation) {
        ...
        const isDbEmpty = await checkDb(process.env.API_POSTGRES_URL);
        if (!isDbEmpty) {
            ...
            if (shouldOverwriteData) {
                await wipeExistingData(process.env.API_POSTGRES_URL);
        ...
        if (shouldStartDockerContainers) {
            ...
            try {
                ...
                while (!isConnected) {
                    ...
                    try {
                        const client = new Client({ connectionString: process.env.API_POSTGRES_URL });
