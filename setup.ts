...
import { loadDefaultOrganization } from "@utilities/loadDefaultOrganization";
...
/**
 * The function `wipeExistingData` deletes all existing data in a PostgresDB database if the database URL is provided.
 * @param url - A string representing the URL of the PostgresDB database.
 * @returns The function returns a Promise that resolves to `void`.
 */
export async function wipeExistingData(): Promise<void> {
    const db = drizzle(pool);

    try {
        // Fetch all table names
        const result = await db.$client.query(`SELECT table_name FROM information_schema.tables WHERE table_schema='public';`);
        ...
        console.error("Could not connect to the database to delete data", error);
    } finally {
        await pool.end();
    }
}
...
/**
 * The function `checkDb` checks if a PostgresDB database is empty by connecting to the database and checking if any
 * collections exist.
 * @param url - A string representing the URL of the PostgresDB database.
...
//Import sample data
/**
 * The function `importData` imports sample data into a PostgresDB database if the database URL is provided and if it
 * is determined that existing data should be wiped.
 * @returns The function returns a Promise that resolves to `void`.
 */
export async function importData(): Promise<void> {
    if (!process.env.POSTGRES_DATABASE_URL) {
        console.log("Couldn't find PostgreSQL database URL");
        return;
    }
...
//Import Default data
/**
 * The function `importDefaultData` imports default data into a PostgresDB database if the database URL is provided.
 * @returns The function returns a Promise that resolves to `void`.
 */
...
// Get the PostgresDB url
/**
 * The `PostgresDB` function manages the connection to a PostgresDB database.
 * It performs the following steps:
 * 1. Checks if there is an existing PostgresDB URL in the environment variables.
 ...
 * 4. If null is returned from the previous step (i.e., no successful connection was made), it prompts the user to enter a new PostgresDB URL.
 * 5. It then enters a loop where it continues to prompt the user for a PostgresDB URL until a successful connection can be made.
 * 6. Once a successful connection is made, it saves the PostgresDB URL to the environment variables.
 * This function is part of the initial setup process and is designed to ensure a valid PostgresDB connection before proceeding.
 */
/**
 * Configure and connect to PostgreSQL database.
 */
export async function postgresDB(): Promise<void> {
    let DB_URL = process.env.POSTGRES_DATABASE_URL || null;

    try {
        let url = await checkExistingPostgres();
        let isConnected = url !== null;

        if (isConnected) {
            console.log("PostgreSQL URL detected: " + url);
            const { keepValues } = await inquirer.prompt({
                type: "confirm",
                name: "keepValues",
                message: `Do you want to connect to the detected PostgreSQL URL?`,
                default: true,
            });

            if (keepValues) {
                console.log("Keeping existing PostgreSQL URL: " + url);
            } else {
                isConnected = false;
            }
        }
        ...
        while (!isConnected) {
            const {host,port,database,user,password} = await askForPostgresUrl();
            isConnected = await createDatabaseAndCheckConnection(host,port,user,password,database );
        }
        ...
        if (process.env.NODE_ENV === "test") {
            ...
            const config = dotenv.parse(fs.readFileSync(".env_test"));
            config.POSTGRES_DATABASE_URL = DB_URL;
            updateEnvVariable(config);
        } else {
            DB_URL = `${url?.endsWith("/talawa-api") ? url : `${url}/talawa-api`}`;
            const config = dotenv.parse(fs.readFileSync(".env"));
            config.POSTGRES_DATABASE_URL = DB_URL;
            process.env.POSTGRES_DATABASE_URL = DB_URL;
            updateEnvVariable(config);
        }
    } catch (err) {
        console.error(err);
...
function getDockerComposeCommand(services: string[] = []): { command: string; args: string[] } {
    let dockerComposeCmd = "docker-compose"; // Default to v1
    let args = ["-f", "compose.yaml", "up", "--build", "-d",...services];
...
async function runDockerComposeWithLogs(): Promise<void> {
    ...
    const services=["api","postgres","minio"];
    return new Promise((resolve, reject) => {
        const { command, args } = getDockerComposeCommand(services);
...
async function main(): Promise<void> {
    ...
    const { isDockerInstallation } = await inquirer.prompt({
        ...
        default: process.env.POSTGRES ? false : true,
    });

    if (isDockerInstallation) {
        if (process.env.POSTGRES_DATABASE_URL) {
            console.log(
                `\nPostgreSQL URL already exists with the value:\n${process.env.POSTGRES_DATABASE_URL}`,
            );

            const { shouldSetupPostgres } = await inquirer.prompt({
                type: "confirm",
                name: "shouldSetupPostgres",
                message: "Would you like to change the existing PostgreSQL URL?",
                default: false,
            });

            if (shouldSetupPostgres) {
                await postgresDB();
            }
        } else {
            await postgresDB();
        ...
        updateEnvVariable(config);
        console.log(`Your PostgreSQL URL is:\n${process.env.POSTGRES_DATABASE_URL}`);
    ...
    const { imageSizeLimit } = await inquirer.prompt(
        {
            ...
            validate: async (input: string): Promise<boolean | string> => {
                ...
                if (validateImageFileSize(size)) return true;
                return `Enter a valid number between 0 and ${MAXIMUM_IMAGE_SIZE_LIMIT_KB / 1000
                    }`;
            }
        },
    );
    ...
    if (!isDockerInstallation) {
        if (!process.env.POSTGRES_DATABASE_URL) {
            console.log("Couldn't find PostgreSQL URL");
            return;
        }

        const isDbEmpty = await checkDb();
        if (!isDbEmpty) {
            ...
            try {
                ...
                while (!isConnected) {
                    if (retryCount >= maxRetries) {
                        throw new Error(
                            "Timed out waiting for PostgreSQL to be ready after 30 seconds"
                        );
                    }
                    try {
                        console.log("hello before postgres connect");
                        await client.connect();
                        console.log("hello after postgres connect");
                        const drizzleClient = drizzle(client);
                        console.log("hello after drizzle connect");
                        await drizzleClient.$client.query('SELECT 1');
                        console.log("hello after drizzle query");
                        await client.end();
                        isConnected = true;
                        ...
                        console.log(
                            `Waiting for PostgreSQL to be ready... Retry ${retryCount + 1}/${maxRetries}. Details: ${error}`
                        );
